...
**토큰 및 예산 추적**  
세 파이프라인 모두 TokenUsageTracker 콜백으로 Bedrock 호출의 입출력 토큰을 추적하며, `max_total_tokens`(설정 시 양수)로 실행당 비용을 상한합니다.
...
PaperEvaluationPrompt(scholar_lens/src/prompts/prompts.py)는 0~100점 척도로 다음을 평가합니다:
