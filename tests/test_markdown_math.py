...
def test_inline_math_underscores_escaped() -> None:
    assert norm("값은 $W_0 + B_1 x$ 이다") == "값은 $W\\_0 + B\\_1 x$ 이다"
...
def test_already_escaped_is_idempotent() -> None:
    assert norm("$a\\_b$") == "$a\\_b$"
    assert norm(norm("$W_0$")) == norm("$W_0$")


def test_currency_dollar_not_treated_as_math() -> None:
    # "$5 ... $10" should not be parsed as a math span swallowing the text.
    out = norm("비용은 5_000 원, 코드 `x_y`")
    assert "5_000" in out  # prose underscore preserved
    assert "`x_y`" in out  # code preserved


def test_mixed_math_and_code_in_one_line() -> None:
    out = norm("수식 $a_b$ 와 코드 `c_d` 와 텍스트 e_f")
    assert "$a\\_b$" in out
    assert "`c_d`" in out
