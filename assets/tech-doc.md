...
Scholar-Lens는 논문 및 기술 문서를 게시 가능한 콘텐츠로 변환하는 AI 기반 연구 보조 도구입니다. 세 가지 아티팩트 유형을 생성하며, CLI, AWS Batch, 또는 Slack 봇을 통해 구동됩니다.
...
#### AWS Batch
프로덕션 워크로드용 컨테이너화 실행. `main()` 함수와 `tech_guide_main.py:_run()` 은 동일한 핵심 로직을 실행하며 (`main.py:80-152`), 선택적으로 SNS 알림 및 Slack 알림 전송 (`main.py:131-151`).
...
**Slack 앱 격리**: Paper Bot은 전용 Slack 앱을 사용해야 합니다 (`README.md:137-138`). 두 개의 Socket Mode 프로세스가 동일한 앱을 공유하면 Slack이 이벤트를 둘 중 하나로만 무작위로 라우팅하므로 멘션이 무시될 수 있습니다. `SLACK_EXPECTED_APP_ID` 환경 변수로 앱 ID 검증 (`main.py:1`에서 불러온 모듈 참고).
...
2. **논문 준비** (`main.py:314-402`의 `_prepare_paper_data()`):
   - PDF 또는 HTML 파싱 (arXiv의 경우 HTML 선호; 임의 URL은 항상 PDF) → 콘텐츠 및 도형 추출.
...
4. **출판** (`Publisher`, `main.py:206-211`):
   - S3에 마크다운 업로드.
   - GitHub 리포지토리에 PR 생성 (Jekyll 프론트 매터 포함).

모든 아티팩트는 configurable 카테고리 레이블 (`tech_guide_main.py:244`)과 함께 블로그 호환 마크다운으로 생성되며, 출력 언어는 구성 파일에서 설정 가능합니다 (`main.py:258`의 `output_language`).
...
생성 에이전트 (ExplainerGraph / PaperSummarizer / TechGuideGenerator)
    ↓
출판 (Publisher) → S3 / GitHub / Slack / SNS
```
...
`normalize_math_underscores()`는 LaTeX 수식 내 underscores를 이스케이프합니다:
...
본 섹션에서는 Scholar-Lens의 세 가지 주요 생성 파이프라인(REVIEW, SUMMARIZE, GUIDE)에서 입력 논문이 최종 산출물로 변환되는 과정을 단계별로 추적합니다.
...
**주요 클래스/함수**:
- `CitationSummarizer` (line 25): `summarize(reference_identifiers, original_content)` (line 85)는 비동기 조회 데디스플리케이션 및 인플라이트 맵(line 55-58)을 사용하여 동일 식별자의 동시 호출을 공유합니다.
...
- `WebResearcher` (line 130): `research(urls, discover_subpages, search_queries)` (line 166)은 각 URL을 페치(SSRF 가드 포함, line 209)하고 빔 조사합니다. `_select_subpages()` (line 263)는 동일 호스트·경로 내 링크만 선택합니다.
...
**주요 클래스/함수**:
- `PaperSummarizer` (line 31): `summarize(paper)` (line 78)은 종이 텍스트에서 `{"summary", "tags", "urls"}`를 추출합니다. 요약은 Markdown 조각(emoji 섹션), 태그는 쉼표로 구분된 키워드, URL은 Markdown 링크입니다.
...
| `paper_finalization_model_id` | `LanguageModelId` | (필수) | 최종 검수 및 포맷팅용 모델 (기본값 없음, 반드시 지정 필요) |
...
| | 논문 풍부화 | Claude 4.5 Sonnet | `anthropic.claude-sonnet-4-5-20250929-v1:0` |
| | 논문 최종화 | Claude 4.5 Haiku | `anthropic.claude-haiku-4-5-20251001-v1:0` |
| | 논문 반성 | Claude 4.5 Sonnet | `anthropic.claude-sonnet-4-5-20250929-v1:0` |
...
리뷰 생성(`ExplainerGraph`)의 반성기(Reflector)와 합성기(Synthesizer)도 독립적으로 사고를 제어할 수 있습니다(`scholar_lens/configs/config.py:144-146`):
...
2. **강화(enrich)**: PaperEnrichmentPrompt로 각 섹션에 필요한 인용(citation_summaries) 및 코드(code) 예제를 선별합니다.
...
- 현재 인덱스에서 단락을 입력받고, 이전 설명(previous_explanation)과 누적된 피드백(accumulated_feedback)을 context로 사용합니다.
...
- **절대 금지**: citation key 노출(예: smith2023transformer, brown2024attention을 prose에 기록 금지).
...
- 모든 삽입된 그림은 주변 prose에서 참조되어야 함(캡션만 있는 고아 이미지 금지).
...
- 자연스럽고 흐르는 prose, 전문적이고 접근 가능한 학술 톤.
...
- **공개 가독성**: `github_config.public_assets` 설정으로 제어 (configs/config.py:45, publisher.py:123). 기본값은 false (프라이빗). GitHub Pages 자산 서빙 모델에서는 S3 공개 ACL이 불필요하고 ブロック公開アクセス 정책이 거부할 수 있으므로, S3에서 직접 서빙하는 경우에만 true로 설정합니다.

### Git 작업 및 PR 생성

PR이 활성화되었을 경우 (schema_lens/main.py:210):
...
**문제**: Paper Bot이 자신의 Slack 앱을 사용해야 하는데, Socket Mode 핸들러가 같은 토큰으로 여러 봇(예: OmniSummary)을 실행하면 Slack이 이벤트를 무작위로 한 프로세스에만 전달하므로 멘션이 잘못된 봇으로 라우팅될 수 있습니다.
...
### Socket Mode 및 전용 앱 요구사항

Paper Bot은 **Socket Mode** (`SocketModeHandler`, bot.py:276)에서만 작동하며, Slack의 실시간 이벤트 수신을 위해 다음이 필수입니다:

- **SLACK_BOT_TOKEN**: 봇 계정에 부여된 xoxb- 토큰 (메시지 송수신 권한)
- **SLACK_APP_TOKEN**: xapp-1- 앱 레벨 토큰 (Socket Mode 연결용)
- **전용 Slack 앱**: 다른 Socket Mode 봇과 토큰을 공유하면 안 됨 (하나의 앱은 하나의 Socket Mode 연결만 지원)
